...
class Conversations {
    ...
    public function __construct() {
        ...
        add_filter('ai_assistant_conversation_export_markdown', [$this, 'filter_export_conversation_as_markdown'], 10, 3);
        add_filter('ai_assistant_conversation_export_html', [$this, 'filter_export_conversation_as_html'], 10, 3);
        add_filter('ai_assistant_conversation_export_json', [$this, 'filter_export_conversation_as_json'], 10, 3);
     ...
     *
     * Export content is generated by the dynamic
     * `ai_assistant_conversation_export_{$format}` filter. The filter may return
     * a string download body, a WP_Error, or an array with content, filename,
     * and/or mime keys.
     */
    ...
    public function admin_post_export_conversation() {
        ...
        $result = $this->export_conversation_with_registered_format($format_slug, $conversation, $format);
        ...
        if (is_array($result)) {
            if (!array_key_exists('content', $result)) {
                wp_die(esc_html__('Export filter did not return content.', 'ai-assistant'), '', ['response' => 500]);
            }
    ...
    private function export_conversation_with_registered_format($format_slug, array $conversation, array $format) {
        $format_slug = sanitize_key($format_slug);

        if ($format_slug === '') {
            return new \WP_Error('invalid_export_format', __('Export format not found.', 'ai-assistant'));
        }

        /**
         * Filters the generated content for a specific conversation export format.
         *
         * The dynamic portion of the hook is the sanitized export format slug.
         * Return null to signal that the format has no registered renderer.
         *
         * @param mixed $result       Export result, or null to continue.
         * @param array $conversation Conversation export data.
         * @param array $format       Export format definition.
         */
        $result = apply_filters("ai_assistant_conversation_export_{$format_slug}", null, $conversation, $format);
    ...
    public function filter_export_conversation_as_json($result, array $conversation, array $format) {
        return $result !== null ? $result : $this->export_conversation_as_json($conversation, $format);
    }
    ...
    public function filter_export_conversation_as_markdown($result, array $conversation, array $format) {
        return $result !== null ? $result : $this->export_conversation_as_markdown($conversation, $format);
    }
    ...
    public function filter_export_conversation_as_html($result, array $conversation, array $format) {
        return $result !== null ? $result : $this->export_conversation_as_html($conversation, $format);
    }
